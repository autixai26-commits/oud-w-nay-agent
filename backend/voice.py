...
# إعدادات مثبتة من مشروع العيادة: بدونها يعود المحرّك للجيم الناعمة
# ويصير النطق أقل ثباتاً. style=0 لأن أي رفع يزيد «التمثيل» على حساب
# ثبات النطق.
VOICE_SETTINGS = {
    "stability": 0.45,
...
# نطلب ogg/opus مباشرةً من المحرّك: تليجرام يوصّف الرسالة الصوتية بأنها
# ملف OGG مرمّز بـOpus، وهذا يلغي الحاجة لتحويل ffmpeg في المسار السعيد.
TTS_OUTPUT_FORMAT = "opus_48000_32"
...
def synthesize(text: str, lang: str = "ar") -> bytes:
    ...
    try:
        with httpx.Client(timeout=_TIMEOUT) as c:
            r = c.post("%s?output_format=%s"
                       % (_TTS_URL % config.ELEVENLABS_VOICE_ID,
                          TTS_OUTPUT_FORMAT),
                       headers=_headers(),
...
def render(text: str, lang: str = "ar") -> bytes:
    ...
    المحرّك يعيد ogg/opus جاهزاً، فنتخطّى ffmpeg في المسار السعيد
    ونبقيه احتياطاً لو عاد بصيغة أخرى.
    """
...
# اللهجة الأردنية: «تمن» لا «ثمانية»، وتتبعها مشتقاتها في كل موضع.
_ONES = ("صفر", "واحد", "اثنين", "ثلاثة", "أربعة", "خمسة", "ستة",
         "سبعة", "تمن", "تسعة", "عشرة", "أحد عشر", "اثنا عشر",
...
_HUNDREDS = {1: "مئة", 2: "مئتين", 3: "ثلاثمئة", 4: "أربعمئة", 5: "خمسمئة",
             6: "ستمئة", 7: "سبعمئة", 8: "تمنمية", 9: "تسعمئة"}
...
def _price_ar(match) -> str:
    ...
    # النصف يُقال «ونص» لا «وخمسمئة فلس»: «سبعة ونص» هي الصيغة المنطوقة
    # في المطعم، والحرفية تُسمع آلية.
    if fils == 500:
        return "%s ونص" % number_words_ar(dinars)
...
# النطق المطلوب صوت G صلب كما في Greece. الحل مأخوذ من مشروع العيادة
# حيث نُطق اسم «جريس» صحيحاً: تُكتب «غ» للمحرّك — أقرب حرف عربي للـG
# الصلبة في النطق الآلي — ويُشترط تمرير voice_settings، فبدونها يعود
# المحرّك للجيم الناعمة. الأمران معاً هما الحل، لا أحدهما.
SHISHA_SPOKEN = "أرغيلة"

PRONUNCIATION_AR = {
    "الأرجيلة": "ال" + SHISHA_SPOKEN,
    "الارجيلة": "ال" + SHISHA_SPOKEN,
    "أرجيلة": SHISHA_SPOKEN,
    "ارجيلة": SHISHA_SPOKEN,
    "أرجيله": SHISHA_SPOKEN,
    "ارجيله": SHISHA_SPOKEN,
}

# الأطول أولاً حتى لا تبتلع «أرجيلة» صورةَ «الأرجيلة».
_PRON_ORDER = sorted(PRONUNCIATION_AR, key=len, reverse=True)
...
def fix_pronunciation(text: str, lang: str = "ar") -> str:
    if lang != "ar" or not text:
        return text or ""
    out = text
    for wrong in _PRON_ORDER:
        out = out.replace(wrong, PRONUNCIATION_AR[wrong])
    return out


# ------------------------------------ تشكيل أسماء المنيو (SPEC 9)
# محرّك الصوت يقرأ الكلمة غير المشكولة بأقرب معنى مألوف له، فـ«مشكلة»
# (متنوّعة) تُقرأ «مُشْكِلة» (problem) و«سلطة» (سَلَطة) قد تُقرأ «سُلْطة».
# نكتب له الصورة المشكولة، والنص المعروض للزبون يبقى كما هو.
#
# القاعدة: تُشكَّل الكلمة كاملةً لا حركة واحدة، لأن حركة مفردة تترك
# الباقي للتخمين. الترتيب من الأطول للأقصر حتى لا يبتلع جزءٌ كلمةً أطول.
MENU_TASHKEEL = {
    # الالتباس الأخطر: متنوّعة مقابل مشكلة
    "مشكلة": "مُشَكَّلة",
    "مشكل": "مُشَكَّل",
    # سَلَطة الطعام لا سُلْطة الحكم
    "سلطة": "سَلَطة",
    # حُمُّص الحبّ لا حِمْص المدينة
    "حمص": "حُمُّص",
    "حميص": "حِمّيص",
    "كبة": "كُبَّة",
    "لبنة": "لَبَنة",
...
# الأطول أولاً: «مشكلة» قبل «مشكل»، و«مقلية» قبل «مقلي».
_TASHKEEL_ORDER = sorted(MENU_TASHKEEL, key=len, reverse=True)


def add_tashkeel(text: str, lang: str = "ar") -> str:
    """يستبدل أسماء المنيو الملتبسة بصورتها المشكولة قبل النطق."""
    if lang != "ar" or not text:
        return text or ""
    out = text
    for bare in _TASHKEEL_ORDER:
        if bare in out:
